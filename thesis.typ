...
= Rubos additional notes
Here are my addditions to this Template.
== Code Blocks
#codly(header: [Code Example mit Titel])
```typst
#let fib(n) = if n <= 1 {
  n
} else {
...
```
```typst
Dieses code-beispiel hat *keinen* Titel.
```
Skip lines
#codly(header: [HelloWorld.java], offset: 5)
```java
// ...
// main method
...
#codly(
  ...
  offset: 5,
  // highlight modifiers for some reason
  highlights: (
    (line: 8, start: 1, end: 6, fill: red),
    (line: 9, start: 5, end: 10, fill: green),
    (line: 9, start: 12, end: 17, fill: teal),
  ),
  ...
  annotations: (
    (
      ...
      content: block(
        ...
        rotate(
          -90deg,
          align(center, box(width: 100pt)[Main function]),
        ),
      ),
    ),
  ),
...
```
== Different fonts
#textsf[das ist seriefenfreie Schrift]

#fatsf[Seriefenfrei und fett]
== Definitions
#definition()[#fatsf[Code-Modus ---] Im Code-Modus können Befehle ohne vorangestelltes `#` verwendet werden. Außerdem markieren Anführungszeichen `"` den Anfang und das Ende eines Strings.]
== Gestylete Itemizes (bin mir gerade nicht sicher, ob das nur in beamer sein sollte)
- asdf
  - bsdf
    - csdf
      - dsdf
...
#[
  ...
  #set table(
    ...
    inset: (xi, yi) => if yi == 0 { (y: 3.5pt, x: 5pt) } else { (y: 2.5pt, x: 5pt) },
  )
  #let tdata = (
  ...
  #figure(
    ...
    scope: "column",
    {
      table(
        ...
        table.header(..tdata.remove(0)),
        ..tdata.flatten(),
      )
    },
    caption: [AST-T5 Performance evaluation on HumanEval+ and MBPP+ benchmarks. The results show that AST-T5 outperforms other models of similar size, achieving state-of-the-art performance on both benchmarks. Adapted from `@the_paper`.],
  )<fig:ast-t5-performance-evaluation>
]
// test content
